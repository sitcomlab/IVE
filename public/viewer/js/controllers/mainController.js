...
app.controller("mainController", function ($scope, $rootScope, $window, config, $routeParams, $filter, $location, $translate, $videoService, $locationService, $overlayService, $sce, $socket, _, $relationshipService) {
    ...
    function changeSource(path, length) {
        ...
        vidload.onloadeddata = () => {
            if (typeof length != "undefined") {
                vidload.playbackRate = vidload.duration/(length/(config.avgWalkingSpeed * config.playbackFactor));
            } else {
        ...
        for (var i = 0; i < $scope.relationships.length; i++) {
            ...
            if ($scope.relationships[i].overlay_category === "website") {
                ...
                if(typeof $scope.current_state != "undefined") {
                    if (typeof $scope.current_state.overlaysstate != "undefined" && !$scope.current_state.overlaysstate) {
                        objectCSS.visible = $scope.current_state.overlaysstate;
                    } else if(typeof $scope.current_state.overlay[$scope.relationships[i].overlay_id] != "undefined") {
            ...
            if ($scope.relationships[i].overlay_category === "picture") {
                ...
                if(typeof $scope.current_state != "undefined") {
                    if (typeof $scope.current_state.overlaysstate != "undefined" && !$scope.current_state.overlaysstate) {
                        object.visible = $scope.current_state.overlaysstate;
                    } else if(typeof $scope.current_state.overlay[$scope.relationships[i].overlay_id] != "undefined") {
     ...
     */
     $socket.on('/get/overlaysstate', function (data) { 
        if ($scope.current_state) {
            $scope.current_state.overlaysstate = data
        } else {
            $scope.current_state = {
                "overlaysstate": data
            }
